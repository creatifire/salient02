...
**Phase 1: Update History Endpoint** (Low risk, immediate benefit)
1. Modify `history_endpoint()` in `account_agents.py`:
   - Remove `HTTPException(401, "No session found")`
   - Return empty array if `session is None`
   - Keep session context update logic for existing sessions
2. Test: Load page → history returns empty array, no session created

**Phase 2: Update Middleware** (Medium risk, prevents vapid sessions)
1. Modify `SimpleSessionMiddleware.dispatch()`:
   - Add check for `/accounts/*/agents/*` routes
   - Skip session creation if no cookie exists on these routes
   - Let chat endpoints handle session creation
2. Test: Load page → no session created, send message → session created with context

**Phase 3: Verify Chat Endpoints** (Low risk, should already work)
