...
1. **BUG-0023-001** (P0): SQLAlchemy concurrent operations error during parallel tool execution 🔴 **IDENTIFIED**
2. **BUG-0023-002** (P1): Configuration cascade looking in wrong path 🟡 **IDENTIFIED**
3. **BUG-0023-003** (P2): Connection pool exhaustion under concurrent load 🟢 **IDENTIFIED**

**Current Impact**: Parallel tool calls → cost tracking failures → lost billing data + abnormally short responses
...
```python
{
  ...
  "llm_request_id": None,  # ← Should be UUID
  "response_length": 24,   # ← Abnormally short (normal: 4000+)
...
```python
{
  ...
  "response_length": 4247  # ← Normal length
}
```
...
- Sequential execution: tool completes → session released → cost tracking begins
- No concurrent access to database session

### Impact

**Data Integrity**:
- ❌ Lost billing data (no cost tracking for these requests)
- ❌ `llm_request_id: None` prevents request tracking
- ❌ Audit trail gaps for parallel tool usage
- ❌ Cannot analyze performance of parallel vs sequential tools

**Financial**:
- ❌ Revenue loss (unbilled LLM usage)
- ❌ Cost analysis incomplete for multi-tool scenarios
- ❌ Cannot verify billing accuracy

**Observability**:
- ❌ Missing data in Logfire/analytics
- ❌ Cannot measure tool calling performance
...
**Pros**:
- ✅ Zero code changes to database layer
- ✅ Prevents all concurrent session conflicts
- ✅ Simple to implement and test
- ✅ No risk of data corruption

**Cons**:
- ❌ Slower responses (tools execute one-by-one)
- ❌ Defeats purpose of parallel tool calling feature
- ❌ Not scalable for complex multi-tool workflows

**Trade-off**: 2.4s (parallel) → ~4.8s (sequential) for 2 tools
...
**Cons**:
- ❌ Requires refactoring dependency injection
- ❌ Updates needed across ~10-15 files
...
- ✅ Preserves parallel tool execution
- ✅ No architecture refactoring needed
- ✅ Low risk, easy to test

**Cons**:
- ❌ Cost tracking delayed until after response
- ❌ If process crashes mid-stream, costs lost
...
- **Detailed Implementation Plan**: See [bugs-0023-001-option-b-revised-plan.md](bugs-0023-001-option-b-revised-plan.md)

**Do Not Do**:
- ❌ **Option A** - Defeats purpose of parallel tool calling
...
- ❌ Log shows: "Cost tracking failed (non-critical)"
- ❌ SQLAlchemy error: "concurrent operations are not permitted"
- ❌ `llm_request_id: None`
- ❌ No record in `llm_requests` table
...
- ✅ No "concurrent operations" errors in logs
- ✅ `llm_request_id` is valid UUID
- ✅ Record exists in `llm_requests` table with correct costs
- ✅ All denormalized fields populated
- ✅ Response length normal (~4000+ chars)
...
## BUG-0023-002: Configuration Cascade Path Resolution Error 🟡 P1
...
**Configuration**:
- ❌ All agents use same model (from global config)
- ❌ Cannot customize temperature per agent
- ❌ Cannot customize max_tokens per agent
- ❌ Agent-specific prompts ignored
- ❌ Agent-specific tool configurations ignored

**Developer Experience**:
- ❌ Confusion: "Why aren't my config changes working?"
- ❌ Per-agent customization broken
- ❌ Multi-tenant architecture partially defeated
...
**Expected Results**:
- ✅ No "Agent configuration file not found" errors
- ✅ Logs show: "config.cascade.success" (or no fallback message)
- ✅ Agent uses model from its own `config.yaml`
- ✅ Agent-specific temperature/max_tokens applied
...
```python
{
  ...
  "model": "openai/gpt-4o-mini",  # ← Should match agent config
  "temperature": 0.3  # ← Should match agent config
}
```
...
**Performance**:
- ⚠️ Requests may wait for available connections
- ⚠️ Timeout errors possible under heavy load
- ⚠️ Increased latency during traffic spikes

**Stability**:
- ⚠️ Pool exhaustion could cause request failures
- ⚠️ May mask underlying session management issues

**Current Observation**:
- ✅ No timeout errors observed yet
- ✅ System functional under current load
...
**Expected Results**:
- ✅ Base pool (20) handles normal load
- ✅ Overflow connections (up to 10) handle bursts
- ✅ No timeout errors under peak load
- ✅ Connections recycled properly after use
...
1. **BUG-0023-001** (P0): SQLAlchemy concurrent operations → Immediate (Option C deferred tracking)
2. **BUG-0023-002** (P1): Configuration cascade path → This week (1-2 hours)
3. **BUG-0023-003** (P2): Connection pool sizing → After BUG-001 fixed, before production
...
**Current Status**:
- 🔴 **BUG-001**: Confirmed via logs, awaiting fix
- 🟡 **BUG-002**: Confirmed via logs, awaiting fix  
- 🟢 **BUG-003**: Suspected issue, needs load testing to confirm

**Success Criteria**:
- ✅ Parallel tool calls complete without cost tracking failures
- ✅ `llm_request_id` is valid UUID for all requests
- ✅ Agent-specific configs loaded correctly (no cascade fallback)
- ✅ Connection pool handles burst traffic without timeouts
