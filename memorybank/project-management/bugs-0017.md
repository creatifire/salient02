...
Update `llm_request_tracker.py` to populate denormalized fields:
...
async def track_request(
    ...
    model: str,
    # ... existing params ...
    agent_instance_id: Optional[UUID] = None,  # Already passed
    account_id: Optional[UUID] = None,  # NEW
    account_slug: Optional[str] = None,  # NEW
    agent_instance_slug: Optional[str] = None,  # NEW
...
**Update callers** in `simple_chat.py` (both streaming and non-streaming):

```python
# Extract account/agent info from instance_metadata or session
account_id = instance_metadata.get("account_id")  # or from session
account_slug = instance_metadata.get("account_slug")  # or from session
agent_instance_slug = instance_metadata.get("instance_slug")
...
llm_request_id = await llm_request_tracker.track_request(
    ...
    completion_status="complete"  # or "partial", "error"
)
```

### Questions

1. **Where to get account_id/account_slug?**
   - From `instance_metadata` passed to `simple_chat()`/`simple_chat_stream()`?
   - From `session` object (already has account_id)?
   - From database lookup using `agent_instance_id`?

2. **Where to get agent_instance_slug?**
   - From `instance_metadata`?
   - Parse from config path?
   - Database lookup?

3. **Where to get agent_type?**
   - From `instance_config["agent_type"]`?
   - Hardcode to "simple_chat" for now?

4. **Should completion_status be an enum?**
   - `"complete"`, `"partial"`, `"error"`, `"timeout"`?
   - Or keep as string for flexibility?

5. **Backward compatibility?**
   - Make new params optional with defaults?
   - Or require them and update all callers?
