...
Eight bugs/issues found during testing:
...
8. **BUG-0017-008** (P2): Code complexity and duplication in simple_chat.py 📋 **PLANNED**
...
## BUG-0017-008: Code Complexity and Duplication in simple_chat.py 📋 PLANNED

### Problem

`simple_chat.py` contains significant code duplication, defensive coding patterns, and mixed responsibilities that reduce maintainability and increase error risk.
...
- Testing complexity: duplicated code paths need parallel test coverage
- Bug risk: fixes may be applied inconsistently
- Readability: long functions obscure core logic
- Onboarding: new developers struggle with code navigation
...
**1.1: Message Building**
```python
def build_request_messages(message_history: List[ModelMessage], current_message: str) -> List[dict]
```
- Consolidates lines 536-558 and 1102-1124
- Handles ModelRequest/ModelResponse conversion to dict format

**1.2: Response Body Construction**
```python
...
```
- Consolidates lines 560-576 and 1126-1145
- Includes provider_details extraction

**1.3: Model Settings Extraction**
```python
async def get_model_settings_for_tracking(
    instance_config: Optional[dict],
    requested_model: str
) -> tuple[dict, str]
```
- Consolidates lines 517-533 and 1074-1099
- Returns (model_settings, tracking_model)

**1.4: Message Pair Saving**
```python
...
```
- Consolidates lines 685-722 and 1227-1261
...
**2.1: Streaming Cost Calculation**
```python
...
```
- Extracts lines 935-1052 (genai-prices + fallback pricing)
- Returns cost_data dict with prompt_cost, completion_cost, total_cost

**2.2: Non-Streaming Cost Extraction**
```python
...
```
- Extracts lines 456-488 (OpenRouter provider_details parsing)
- Returns (prompt_cost, completion_cost, total_cost)

**2.3: LLM Request Tracking Wrapper**
```python
...
```
- Consolidates lines 658-683 and 1191-1217
- Handles session account field extraction internally
- Returns llm_request_id or None

#### **Phase 3: Simplify Defensive Code**

**3.1: Remove Paranoid Type Checking**
- Delete `safe_type_name()`, `safe_repr()` (lines 617-633)
- Rely on `logfire_safe.py` wrappers only in exception handlers
- Trust that `session_extractor.py` returns Python primitives

**3.2: Simplify Account ID Conversion**
```python
# Replace lines 371-394 with:
session_deps.account_id = UUID(str(account_id)) if account_id else None
```
- `session_extractor.py` guarantees primitives, no defensive conversion needed

**3.3: Streamline Exception Handlers**
- Replace lines 726-766 with direct `safe_logfire_error()` call
- Remove multiple try/except for string conversions
- Trust exception serialization to `logfire_safe.py`

#### **Phase 4: Refactor Main Functions**

**4.1: Simplify `simple_chat()`**
- Target: 150-200 lines (current: 464)
- Structure:
  1. Setup (deps, history, agent)
  2. Execute agent
  3. Extract costs (helper function)
  4. Track request (helper function)
  5. Save messages (helper function)
  6. Return response

**4.2: Simplify `simple_chat_stream()`**
- Target: 150-200 lines (current: 526)
- Structure:
  1. Setup (deps, history, agent)
  2. Stream agent execution
  3. Calculate costs (helper function)
  4. Track request (helper function)
  5. Save messages (helper function)
  6. Yield completion

**4.3: Extract Agent Creation Logic**
- Move lines 115-309 (`create_simple_chat_agent`) to separate file
- Create `backend/app/agents/agent_factory.py`

#### **Phase 5: Configuration Loading**

**5.1: Consolidate Config Access**
```python
@dataclass
...
**5.2: Centralize History Loading**
- Already exists but ensure single call point in both functions
...
- Test refactored `simple_chat()` and `simple_chat_stream()`
- Verify identical behavior to original implementation
- Test error paths and edge cases

**6.3: Manual Testing**
- Run all demo agents (wyckoff, agrofresh, prepexcellence, default_account)
- Verify cost tracking accuracy
- Verify message saving and history loading
- Check Logfire for proper event logging

### Files to Create/Modify

**New Files**:
- `backend/app/agents/chat_helpers.py` (Phase 1)
- `backend/app/agents/cost_calculator.py` (Phase 2)
- `backend/app/agents/agent_factory.py` (Phase 4.3)
- `backend/tests/unit/test_chat_helpers.py` (Phase 6)
- `backend/tests/unit/test_cost_calculator.py` (Phase 6)

**Modified Files**:
- `backend/app/agents/simple_chat.py` (all phases)
- `backend/app/services/llm_request_tracker.py` (if interface changes needed)
...
- Potential optimization opportunities become visible

**Future-Proofing**:
- Easier to add new agent types
- Helper functions reusable across agents
...
- All critical bugs fixed (BUG-0017-001 through BUG-0017-006)
- Refactoring is technical debt reduction
- Should complete before adding new agent types

### Dependencies

- None - can proceed independently
- Recommend after BUG-0017-007 (legacy endpoint removal)
