...
## 🎯 Epic 0025 - Dynamic Prompting Implementation (Phases 1-4A)
...
**Epic Objective**: Schema-driven dynamic prompt generation for Pydantic AI agents, enabling multi-tool support, multi-directory selection, and domain-agnostic tool documentation.

**Strategy**: Incremental 7-phase implementation with zero breaking changes and 100% backward compatibility at each phase.
...
**Objective**: Implement multi-tool support using Pydantic AI's native `FunctionToolset`.
...
- Foundation ready for subsequent phases

**Benefits**: 🔧 Cleaner architecture | 🎯 Multi-tool foundation | 📦 Modular toolset management

---

### 0025-002 - FEATURE - Phone Directory Prerequisites (Phase 2)
**Status**: 📋 Planned
...
**Deliverable**: Wyckoff agent with TWO directory types for Phase 3 validation

**Benefits**: 🏥 Immediate business value | ✅ Real multi-directory scenario | 🧪 Phase 3 validation data

---

### 0025-003 - FEATURE - Schema Standardization + Multi-Directory Selection (Phase 3)
**Status**: 📋 Planned

**Objective**: Enable domain-agnostic prompt generation and intelligent multi-directory selection via schemas.

**Prerequisites**: Phase 2 complete (phone_directory data must exist)

**Planned Tasks**:
1. Update `medical_professional.yaml` with `directory_purpose` section
2. Add `synonym_mappings_heading` to search_strategy
3. Rename `medical_specialties` → `formal_terms` (standardized key)
4. Create `prompt_generator.py` module (domain-agnostic)
5. Update `simple_chat.py` to dynamically enhance system prompts
6. E2E testing with 6 multi-directory queries

**Key Features**:
- `directory_purpose`: description, use_for, example_queries, not_for
- Schema-driven Directory Selection Guide for LLM
- Multi-directory query guidance
- Zero code changes to add new directory types

**Deliverable**: LLM intelligently selects correct directory (doctors vs phone_directory) based on query intent

**Benefits**: 🧠 Intelligent directory selection | 🔄 Schema-only directory additions | 🌐 Domain-agnostic prompts

---

### 0025-004 - FEATURE - Multi-Tool Testing (Phase 4A)
**Status**: 📋 Planned

**Objective**: Validate that multiple toolsets (directory + vector) work together correctly without conflicts.

**Prerequisites**: Phase 1 complete (toolsets infrastructure)

**Independent of**: Phase 4B (prompt caching) - Pure functional validation

**Planned Tasks**:
1. Configure Wyckoff agent with both directory and vector tools enabled
2. Create comprehensive test suite (10 scenarios)
3. Validate tool selection accuracy (>80% target)
4. Performance benchmarking
5. Regression testing for all agent types

**Test Scenarios** (10 types):
- Directory-only queries (3)
- Vector-only queries (3)
- Ambiguous queries requiring both tools (2)
- Sequential tool usage (1)
- Error handling (1)

**Deliverable**: Multi-tool infrastructure validated, confidence in multi-tool agents

**Benefits**: ✅ Functional validation | 📊 Performance baselines | 🔒 Regression confidence

---

**Phase 1-4A Expected Results**:
- ✅ Multi-tool support (directory + vector + future MCP servers)
- ✅ Intelligent multi-directory selection (LLM chooses correct directory)
- ✅ Zero code changes to add new directory types (schema + CSV only)
- ✅ 100% backward compatibility maintained

**Next Phases**: 4B (Prompt Caching), 5 (Modular Prompts), 6 (MCP Integration)

---

## 0023-009 - FEATURE - Phone Directory for Hospital Departments
**Status**: 📋 Planned (See Phase 2 above - 0025-002)
